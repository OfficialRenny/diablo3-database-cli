...
d3client.getItem = function(url, cb) {
  h.http(url, function(e, body) {
    ...
    var item = {
      ...
      desc2: $('div[class=db-flavor-text]').text().trim(),
      ...
      set: {
        ...
        pieces: $('li[class*=item-itemset-piece]').map(function() {
                  var piece = {
                    name: $(this).text().trim(),
                    link: $(this).find('a').attr('href'),
                  };
                  piece.link = config.BASE_URL + piece.link;
                  piece.id = piece.link.split('/').pop();
                  return piece;
                }).get(),
        bonus: $('li[class*=item-itemset-bonus-amount]').map(function() {
                 return {
                   count: +$(this).find('span[class=value]').text().trim(),
                   desc:  $(this).nextUntil('li[class*=item-itemset-bonus-amount]').map(function() {
                            return $(this).text().trim();
                          }).get(),
                 };
               }).get(),
      ...
      source: {
        ...
        materials: $('div[class=recipe-materials] >a').map(function() {
                     var material = {
                       count: +$(this).find('span[class=d3-num]').text().trim(),
                       link:  $(this).attr('href'),
                       icon:  h.url($(this).find('>span >span >span').attr('style')),
                     };
                     material.link = config.BASE_URL + material.link;
                     material.id = material.link.split('/').pop();
                     return material;
                   }).get(),
      },
    };

  ...
  h.http(url, function(e, body) {
    ...
    var item = {
      ...
      attrs: {
        ...
        legendaryeffect: $('ul[class=item-effects] >span[class=d3-color-ffff8000],span[class=d3-color-ffff0000],span[class=d3-color-ff9b9b9b]').map(function() {
                   return $(this).d3_text();
                 }).get(),
      },
      ...
      set: {
        ...
        bonus: $('span[class=d3-color-ff00ff00]').map(function() {
                 return $(this).d3_text();
               }).get(),
      },
...
d3client.getHeroes = function(url, cb) {
  h.http(url, function(e, body) {
    if (e) return cb(e);

    var $ = h.cheerio(cheerio.load(body));

    var heroes = $('ul[class=hero-tabs] li >a').map(function() {
      var hero = {
        id:    $(this).attr('href'),
        level: $(this).find('>span[class=level]').d3_text(),
        name:  $(this).find('>span[class=name]').d3_text(),
      };

      $(this).attr('class').split(' ').forEach(function(s) {
        if (!s.endsWith('male')) return;

        var parts = s.split('-');
        hero.gender = parts.pop();
        hero.owner = parts.join('-');
      });

      return hero;
    }).get();

    return cb(null, heroes);
  });
};
